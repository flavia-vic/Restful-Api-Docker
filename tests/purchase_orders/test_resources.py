...
def test_delete_purchase_order(test_client, get_headers, seed_db):
    response = test_client.delete(
        f'/purchase_orders/{seed_db.id}', headers=get_headers)

    assert response.status_code == 200
    assert response.json['message'] == f'Pedido de id {seed_db.id} removido com sucesso'


def test_delete_nonexistent_purchase_order(test_client, get_headers):
    id = 9999
    response = test_client.delete(f'/purchase_orders/{id}', headers=get_headers)
